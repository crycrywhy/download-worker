...
Which files go over (2026-09-14): worker.log (+ its rotations) plus the two supervisor
logs and the tunnel log.  The tunnel / supervisor logs are the ones that answer
"why did the outlet die at 03:12" without a human having to log into the PC - exactly
the question the 2026-09-14 incidents kept raising.
"""
import hashlib
import json
import subprocess
...
def ensure_remote_dir():
    ...
    created yet (or was moved by a re-organisation) - the state that shipped until
    2026-09-14: the task ran, `cat >>` failed, and nothing ever arrived.
    """
