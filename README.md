...
适用场景：服务器直连某些外部数据源慢或被限流，而 Windows 家宽出口快得多。
...
| **`tools/`** | **Linux 侧下载链**：提交 → 排队 → 派发 → 传输 → 看状态。出口可以是你自己的机器，也可以是本仓的 Windows Worker | 本文件 [Linux 侧下载链](#linux-侧下载链) |
| **`mcp/`** | **MCP 适配层**：把这条下载链包装成 MCP Server，任何 MCP 客户端都能用工具调用发起下载、查进度、看出口健康状况 | [`mcp/README.md`](mcp/README.md) |
...
四者可以分开用：只想要「Windows 家宽当下载出口」，看完本文件就够了；
想在 Linux 上跑一条带队列与续传的下载链，看 `tools/`；
...
`tools/` 是一条自带队列的下载链：申请写进队列 → driver 按优先级派发 → 下载器
分块传输并逐块校验 → 落地后做终验（大小 / 校验和 / 空洞扫描），全过才算完成。
传输可以走本仓的 Windows 出口（更快），也可以直连目标站点。

```bash
# 配置：仓库里不带任何站点真值，部署差异全在这一步填
...
python3 tools/dl_config.py --check

# 起 driver（长驻：它才是把队列里的申请变成实际下载的那个进程）
python3 tools/dl_all_priority.py

# 提交一个下载，然后看进度
python3 tools/dl_req.py submit --item "item-001" --platform gamma \
    --url "https://example.org/archive.bin" --outdir /abs/path/item-001
python3 tools/dl_req.py list
python3 tools/dl_ctl.py show               # 队列 / 在跑的线 / 运行时控制
```

几个值得先知道的点：

* **等级**：`p0` 加急 / `p1` 高 / `p2` 普通（默认）/ `p3` 低 / `p4` 批量。
  等级高的先跑；**同等级内后提交的先跑**。提交时用 `--level`。
* **运行时控制不用重启**：`dl_ctl.py` 能改线数、暂停/恢复、插队、跳过某个目标，
  写的是控制文件，driver 下一轮就读到。
* **中断了就重跑同一条命令**：断点记在 sidecar（`<输出>.download.json`）里。
  sidecar 还在就说明没下完 —— 别手动删输出文件。
* **终验全过才算完成**：字节数对、校验和对，都不代表中间没有整段空洞；
  这条链默认会扫空洞并把有洞的文件隔离待重下。
* **出口**：没配 worker 时用 `--direct` 直连；配了则默认走出口、失败可回落。
  出口侧（Windows Worker）的安装见本文件上文。

完整参数与口径见 [`mcp/用户手册.md`](mcp/用户手册.md)（那份手册同时是这条链的用户手册）。
...
| `-WorkerPort` | `<端口>` | Worker HTTP 端口（Windows 本机） |
...
| `-LogSyncRemoteDir` | `/home/<LinuxUser>/<工具目录>/log` | 日志同步到 Linux 的目标目录 |
| `-LinuxWorkerPoolPath` | `<工具目录>/worker_pool.py` | Linux 侧 worker 池脚本路径。`download-worker on/off` 通过它同步本机在池里的启用状态（见「手动开关」） |
| `-LinuxDwTasksPath` | `<工具目录>/dw_tasks.py` | Linux 侧只读任务视图路径。`download-worker status` / `process` 通过它显示 Linux 的下载队列与进度（见「[Linux 侧任务视图](#linux-侧任务视图dw_taskspy)」） |
...
```json
{
  ...
  "linux_worker_pool_path": "<工具目录>/worker_pool.py",
  "log_sync": {
    "enabled": true,
    "remote_dir": "/home/<LinuxUser>/<工具目录>/log",
    "interval_minutes": 5
  },
  "proxy": {
    "url": "http://<代理IP>:<端口>",
...
> **为什么隧道也要守护**（一次真实事故）：任务计划自带的重启只在**任务失败退出**时生效；ssh 若自己正常退出（换网络 / 掉线 / 被对端断开），任务状态会停在 `Ready` 而没人拉它 —— 表现就是「PC 在线、worker 也在，但 Linux 侧那个端口连不上」。守护脚本把「拉起 ssh」变成一个永不退出的循环，同时它也顺带做到「开机/登录后一定会有隧道」。两件事都由计划任务 + 守护脚本闭环。
...
>
> **退回 `Interactive` 还有一个更隐蔽的后果**：任务进程会落在"启动它的那个登录会话"里——如果那个会话是 SSH / RDP，**退出该会话时 Windows 会把隧道一起带走**（实测症状：从 shell 登录再 exit，worker 的隧道跟着关）。安装器会在安装日志里回读并打印 `LogonType = S4U/Interactive`，`verify.cmd` 也会显示，一眼可查；要修就给该账号"作为批处理作业登录"权限（`secpol.msc` → 本地策略 → 用户权限分配）后重跑安装器。

> 第三项 `Local Download Log Sync` 是**短命任务**：每 N 分钟（默认 5）启动一次，一秒钟左右同步完就退出，所以平时去看它永远是 `Ready`——那是正常的，不是没跑。
> 它把 `worker.log`（含 `.1`–`.5` 轮转）以及 `tunnel.log`、`worker_supervisor.log`、`tunnel_supervisor.log` **增量**推回 Linux 侧的 `log_sync.remote_dir`（断点存在 `logs\worker.log.sync-state`）；远端目录不存在时脚本会先 `mkdir -p` 建好（此前远端目录一缺就静默失败、日志从没到过 Linux 侧）。不需要这个任务就用 `-SkipLogSyncTask` 关掉。
...
| 默认目标 | 省略 url 时用内置的公开测速文件（`proof.ovh.net/files/10Mb.dat`，支持 Range、无需凭据） |
...
| 只看本机 | Linux 段按**本机的隧道口**过滤（`-R <LinuxTunnelPort>`，每台 PC 各一个），经 `dw_tasks.py --channel` 实现：每台 PC 只看到自己这条口在下的任务，另有「全局在下 N 个」一行说明全池情况。「最近完成」是全局的（台账无 PC 归属），会标注出来 |
...
> 需要 Linux 侧存在 `dw_tasks.py`（默认 `<工具目录>/dw_tasks.py`，即本仓库的 `linux/dw_tasks.py`；路径可用安装参数 `-LinuxDwTasksPath` 改）。没有它时 `process` 与 `status` 的 Linux 段只会打一行提示，**其余功能不受影响**。
...
* **Linux 侧等价操作**（不碰 Windows 时也可用）：`python3 <工具目录>/worker_pool.py --set-enabled <端口> false`，或 `--get-enabled <端口>` 查。
...
```text
  [ OK ]  下载出口：WORKER_PROXY（本机独立代理）—— <代理IP>:<端口> → <目标站> 
...
python3 dw_tasks.py --channel <口>    # 只看这个通道口（某台 PC 的隧道口）在下的任务
```

**通道归属（`--channel` / 帧里的 `channel` 字段）**：`linux_downloader.py` 的 `--worker http://127.0.0.1:<口>`
决定一条线走哪台 PC，所以按进程命令行就能把「此刻在下的任务」分到 `linux`（服务器直连）/ `<隧道口>` 各口
（扫 `/proc` 得到，只读）。`--channel <口>` 只保留该口的在下任务；**过滤只作用于列表**，
帧里的 `active_total` / `active_live_total` 仍是过滤前的全局条数 —— Windows 侧据此同时显示
「本机 N 个 / 全局 M 个」，不会被误读成全局只剩这几条。**完成记录分不了 PC**（台账 CSV 没有「哪个口下的」这一列），
「最近完成」始终是全局口径并如实标注。端口不写死：将来多一台 PC，新的口自动成为一种通道。

> **升级顺序**：`--channel` 是较新才有的参数，Windows 侧的 `download-worker.ps1` 会直接带上它。
> 所以**先更新 Linux 侧的 `dw_tasks.py`，再换 Windows 侧的脚本** —— 反过来的话，老 `dw_tasks.py`
> 遇到不认识的参数会直接报错退出，那一块就显示成「没有输出」（其余功能不受影响）。

`--watch` 是 JSONL：一行一个对象、写完即 flush，管道断开（Ctrl+C / ssh 掉线）就退出，不会僵在那里。列名认得几种常见写法（`name`/`item`/`label`、`size_gb`/`size`…，完整对照见 `dw_tasks.py` 的 `COLUMN_ALIASES`），缺列只是少一项信息，不会报错。

## HTTP API

Worker 是 FastAPI 应用，默认监听 `http://<Tailscale IP>:<WorkerPort>`；经隧道后从 Linux 侧访问 `http://127.0.0.1:<LinuxTunnelPort>`。
...
# 让 Windows 机器自己下载并保存到它自己的盘
curl -s -X POST http://127.0.0.1:<端口>/download \
     -H "Content-Type: application/json" \
     -d '{"url":"https://example.org/big.tar","output":"<盘符>:\\Downloads\\big.tar"}'
```
...
// 在传：字段全部来自本次请求
{"state": "downloading",
 "url":    "https://example.org/archive.bin",
 "task":   "example-task-0001",    // 请求头 X-Task，没发就是空串
...
linux_downloader.py
   │  http://127.0.0.1:<端口>
   ▼
隧道 ssh ──── Tailscale ────► worker.py ──► WORKER_PROXY ──► <代理IP>:<端口> ──► 目标站
      -R <端口>(worker)            │              （本机直接连，不经隧道）
                                 └─► DIRECT ────► 目标站（代理不可达时的兜底）
```

**日志**：Worker 每次请求都写一行 `[NETWORK] plan=WORKER_PROXY,DIRECT url=<去 query 的 URL> range=<...>`，实际用哪条出口由结果行给出：

```text
[NETWORK] route=WORKER_PROXY url=https://example.org/... status=206 range=bytes=0-... proxy=<代理IP>:<端口>
