...
.DESCRIPTION
    ...
    step and a later uninstall fail (hand-uninstall notes, 2026-09-14).
...
.PARAMETER LogSyncRemoteDir
    ...
    Default: /home/<LinuxUser>/script/downloads/log
...
.PARAMETER KeepPaused
    ...
    healthy" (PC2, 2026-09-14).  By default the installer removes the flag and
...
.EXAMPLE
    .\install-worker.ps1 -InstallDir "D:\LocalDownloadWorker" -LinuxUser <user> `
        -LinuxHost <linux-ip> -LinuxTunnelPort 8767
#>
...
param(
    ...
    # (exit 255, endless retry: exactly what PC2 showed on 2026-09-14).
...
function Install-DownloadWorkerSwitch {
    # `download-worker on|off|status` from any terminal (user request, 2026-09-14).
    #
...
function Register-HiddenTask {
    ...
    try {
        ...
        Write-Log "  the exact failure reported on 2026-09-14. Fix: grant this account" "WARN"
...
# --------------------------------------------------------------------------
# previous-installation cleanup (2026-09-14)
#
# Why this exists (the hand-uninstall notes of 2026-09-14, lab PC): removing an old by-hand install
# hand failed twice - first because venv python.exe processes held the files, then
...
function Get-InstallTunnelPorts {
    ...
    # port this install will use, and 8766 (the first version's default).
    param([string[]]$Dirs)
...
function Get-TunnelSshProcesses {
    ...
    # contain "-R <port>:<host>:<port> <user>@<linux>" (2026-09-14 uninstall notes).
...
function Stop-ExistingTasks {
    ...
    # from a previous installer) in place - which is exactly what a 2026-09-14 acceptance
    # run found on the lab PC: Tunnel=Interactive while Worker=S4U.  Stopping first makes
...
function Register-WorkerTasks {
    ...
    # failure that left one machine's outlet offline for hours on 2026-09-14.
    $action = New-ScheduledTaskAction `
        -Execute $VenvPythonw `
...
function Get-TunnelOwnership {
    ...
    # the tunnel is up (PC2, 2026-09-14).  Ownership has to be decided by ancestry.
...
function Start-AndVerifyTunnel {
    ...
    # Since 2026-09-14 the task runs tunnel_supervisor.py, which in turn keeps
    ...
    # this check - which only counted processes - still reported OK (PC2, 2026-09-14).
...
Pick a free port per PC:  -LinuxTunnelPort 8767
...
try {
    ...
    # switched off (PC2, 2026-09-14).  Installing is an explicit "make it work again"
    ...
    if ($null -ne $existing) {
        ...
        # linux_worker_pool_path only exists in configs written by the 2026-09-14 installer
...
"@
    ...
    Write-WorkerConfig -WorkerHost $tailscaleIp -ResolvedLinuxUser $linuxUser -ResolvedLinuxHost $linuxHost
